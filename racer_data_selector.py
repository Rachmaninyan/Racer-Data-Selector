...
def read_file(filename: str) -> list[RaceResult]:
    ...
    with open(filename, 'r') as file:
        ...
        for line in lines:
            ...
            minutes = int(line[TIME_MS])//60000
            remainder = int(line[TIME_MS])-(minutes*60000)
            seconds = remainder//1000
            milliseconds = remainder - (seconds*1000)
            outputlist.append(RaceResult(line[NAME],line[COUNTRY],RaceTime(milliseconds,seconds,minutes)))
    return outputlist
...
def find_athlete(loresults: list[RaceResult], name: str) -> int:
    ...
    """
    found = -1
    for i in range(len(loresults)):
        if loresults[i].get_name() == name:
            if found!=-1:
                return found
            else: 
                found = i
...
def get_all_from_country(loresults: list[RaceResult], country: str
    ...
    """
    outputlist = []
    for raceresult in loresults:
        if raceresult.get_country() == country:
            outputlist.append(raceresult)
    return outputlist
...
def get_fastest_time(loresults: list[RaceResult]) -> RaceTime:
    ...
    for i in range(1,len(loresults)):
        if loresults[i].get_finish_time().is_faster(loresults[i-1].get_finish_time()):
            fastest_result = loresults[i]
...
def get_with_fastest_time(loresults: list[RaceResult]
    ...
    for result in loresults:
        if result.get_finish_time() == fastest_time:
            outputlist.append((result.get_name(),result.get_country()))
    return outputlist